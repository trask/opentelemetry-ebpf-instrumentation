...
func (s *routeGuideServer) GetFeature(_ context.Context, point *pb.Point) (*pb.Feature, error) {
	fmt.Println("GetFeature lat %v long %v\n", point.Latitude, point.Longitude)
...
func (s *routeGuideServer) ListFeatures(rect *pb.Rectangle, stream pb.RouteGuide_ListFeaturesServer) error {
	fmt.Printf("ListFeatures rect %v\n", rect)
	for _, feature := range s.savedFeatures {
...
func (s *routeGuideServer) loadFeatures(filePath string) {
	...
	if filePath != "" {
		...
		if err != nil {
			fmt.Printf("Failed to load default features %w\n", err)
			os.Exit(-1)
		}
	} else {
		data = exampleData
...
func Setup() error {
	...
	if *tls {
		...
		if err != nil {
			fmt.Printf("Failed to generate credentials %w\n", err)
			return err
		}
	...
	fmt.Printf("GRPC listening and serving on port %d\n", *port)
	err = grpcServer.Serve(lis)
	if err != nil {
		fmt.Printf("failed to serve %w\n", err)
		return err
	}
	return nil
}
